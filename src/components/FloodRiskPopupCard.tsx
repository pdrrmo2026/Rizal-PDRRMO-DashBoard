...
import { X, Waves, Users, School, Building2, AlertTriangle, AreaChart, Clock } from 'lucide-react';
import { FloodRiskData } from '../services/floodRiskService';

interface FloodRiskPopupCardProps {
  data: FloodRiskData;
  isOpen: boolean;
  onClose: () => void;
}

export default function FloodRiskPopupCard({ data, isOpen, onClose }: FloodRiskPopupCardProps) {
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-slate-800 bg-gradient-to-r from-slate-900 to-slate-800/50 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`p-2.5 rounded-xl ${risk.bg} ${risk.border} border shadow-lg`}>
              <Waves className={`w-6 h-6 ${risk.color}`} />
            </div>
            <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-4 backdrop-blur-sm">
              <div className="flex items-center gap-3 mb-2">
                ...
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Flood-Prone Area</p>
              </div>
              <p className="text-2xl font-bold text-blue-400">{data.floodProneArea}</p>
              <p className="text-xs text-slate-500 mt-1">{data.floodPronePercentage} of total land</p>
            </div>
            <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-4 backdrop-blur-sm">
              <div className="flex items-center gap-3 mb-2">
                <AlertTriangle className={`w-4 h-4 ${risk.color}`} />
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Susceptibility</p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {veryHighRiskBarangays.map((b, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-rose-500/5 border border-rose-500/20 rounded-lg group hover:bg-rose-500/10 transition-colors">
                  ...
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-rose-500 text-white shadow-sm">VERY HIGH</span>
                </div>
              ))}
              {highRiskBarangays.map((b, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-amber-500/5 border border-amber-500/20 rounded-lg group hover:bg-amber-500/10 transition-colors">
                  <span className="text-sm font-medium text-slate-200">{b.name}</span>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-amber-500 text-white shadow-sm">HIGH</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="px-6 py-4 border-t border-slate-800 bg-slate-950/30 text-center">
          <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">
            Data provided by GeoRiskPH · Geologic and Hydrometeorological Hazard Analytics
          </p>
        </div>
      </div>
    </div>
